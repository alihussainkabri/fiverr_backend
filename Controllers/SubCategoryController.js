...
router.list = async (req,res) => {
    ...
    if (req.user_data.is_admin){
        await knex('sub_categories').where('status','!=',4).orderBy('id','desc').then(response=>{
            if (response){
                status = 200;
                message = 'sub category list has been fetched successfully!'
...
router.create = async (req,res) => {
    ...
    if (req.user_data.is_admin){
        ...
        await knex('sub_categories').insert(create_obj).then(response=>{
            if (response){
                status = 200;
...
router.getDetailById = async (req,res) => {
    ...
    if (req.user_data.is_admin){
        await knex('sub_categories').where('id',id).then(response=>{
...
router.update = async (req,res) => {
    ...
    if (req.user_data.is_admin){
        await knex('sub_categories').where('id',id).update({
            subcategory_name : inputs.name,
            category_name : inputs.category_name,
            category_id : inputs.category_id,
            ...
            if (response){
                status = 200;
                message = 'sub category has been edited succesfully!'
            }
        }).catch(err=>console.log(err))
    }else{
        status = 401;
